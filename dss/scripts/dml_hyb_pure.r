...
data_names = parental_group$id
hybrids = parental_group |> filter(hybrid) |> pull(id)
non_hybrids = parental_group |> filter(!hybrid) |> pull(id)
...
dml_test = bsseq_data |> try_dml(non_hybrids, hybrids, no_output = TRUE)
